...
import {
  FOOTER_COLOR_PRESETS,
  parseCustomFooterColor,
  serializeCustomFooterColor,
} from "@/lib/footer-colors";
...
export function FooterColorPicker({ value, onChange }: Props) {
  ...
  const [fromHex, setFromHex] = useState(custom?.from ?? "#2b6cb0");
  ...
  useEffect(() => {
    const parsed = parseCustomFooterColor(value);
    if (parsed) {
    ...
    const preset = FOOTER_COLOR_PRESETS.find((p) => p.id === value);
    if (preset) {
  ...
  function applyCustom(nextMode: "solid" | "gradient", from: string, to: string) {
    if (nextMode === "solid") {
      onChange(serializeCustomFooterColor({ kind: "solid", from }));
    } else {
      onChange(serializeCustomFooterColor({ kind: "gradient", from, to }));
    }
  }

  function handleEnableCustom() {
    applyCustom(mode, fromHex, toHex);
  }
